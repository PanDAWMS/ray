...
class TestTaskStatus:
    def test_save_restore_status(
        self, nfiles, tmp_path, config, sample_job, sample_ranges
    ):
        ...
        for i in range(0, n_output_per_input_file):
            ...
            for j in range(hits_per_file):
                ranges_list.append(
                    ranges[file_no + (i * offset) + (i + j) * offset]
                )
            ranges_map = {}
            ...
            for r in ranges_list:
                ...
                ranges_map[event_range.eventRangeID] = (
                    TaskStatus.build_eventrange_dict(event_range, fname)
                )
    ...
    def test_set_processed(
        self, nfiles, nevents, tmp_path, config, sample_job, sample_ranges
    ):
        config.ray["outputdir"] = tmp_path
        job = PandaJob(list(sample_job.values())[0])
        ts = TaskStatus(job, tmp_path, config)
        ...
        for r in ranges_list:
            ts.set_eventrange_simulated(
                EventRange.build_from_dict(r), "outputfile"
            )
            ...
            for r in ranges_list:
                ...
                ranges_map[event_range.eventRangeID] = (
                    TaskStatus.build_eventrange_dict(
                        event_range, f"outputfile-{event_range.eventRangeID}"
                    )
                )
