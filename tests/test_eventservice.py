...
class TestEventRangeRequest:
    ...
    def test_from_dict_init(self):
        ...
        for pandaID, req in request_dict.items():
            ranges_request_init.add_event_request(pandaID, req['nRanges'],
                                                  req['taskID'],
                                                  req['jobsetID'])
        assert len(request_dict) == len(ranges_request) == len(
            ranges_request_init)
        for id1, id2, id3 in zip(ranges_request, ranges_request_init,
                                 request_dict):
            assert ranges_request[id1]['pandaID'] == ranges_request_init[id2][
                'pandaID'] == request_dict[id3]['pandaID']


class TestEventRangeUpdate:

    def test_build_range_update(self, nevents, sample_rangeupdate,
...
class TestEventRangeQueue:
    ...
    def test_new(self, nevents, sample_job, sample_ranges):
        ...
        assert len(ranges) == len(
            ranges_queue) == ranges_queue.nranges_available() ==\
            ranges_queue.nranges_remaining() == nevents
        assert ranges_queue.nranges_assigned() == ranges_queue.nranges_done() ==\
            ranges_queue.nranges_failed() == 0
    ...
    def test_concat(self, nevents, sample_job, sample_ranges):
        ...
        assert len(ranges) == len(ranges_queue) ==\
            ranges_queue.nranges_available() ==\
            ranges_queue.nranges_remaining() == nevents
        assert ranges_queue.nranges_assigned() ==\
            ranges_queue.nranges_done() ==\
            ranges_queue.nranges_failed() == 0
        assert ranges_queue[
            ranges[0]['eventRangeID']].eventRangeID == ranges[0]['eventRangeID']
        for r in ranges:
            assert r['eventRangeID'] in ranges_queue

    def test_update(self, sample_job, sample_ranges, nevents,
                    sample_rangeupdate, sample_failed_rangeupdate):
        pandaID = "0"
        ranges = list(sample_ranges.values())[0]
        ranges_queue = EventRangeQueue.build_from_list(ranges)
        ...
        ranges_update = EventRangeUpdate.build_from_dict(
            pandaID, sample_rangeupdate)[pandaID][:nsuccess]
...
class TestPandaJobQueue:
    ...
    def test_build_pandajob_queue(self, is_eventservice, njobs,
                                  sample_multijobs):
        assert len(sample_multijobs) == njobs
        pandajob_queue = PandaJobQueue()
        pandajob_queue_fromdict = PandaJobQueue.build_from_dict(
            sample_multijobs)
    ...
    def test_pandajob_process_event_ranges_reply(self, is_eventservice, njobs,
                                                 sample_multijobs,
                                                 sample_ranges):
    ...
    def test_process_event_ranges_update(self, is_eventservice, njobs, nevents,
                                         sample_multijobs, sample_ranges,
                                         sample_rangeupdate):
        if not is_eventservice:
            pytest.skip("Not eventservice jobs")
        pandajob_queue = PandaJobQueue(sample_multijobs)
...
class TestPandaJobRequest:
    ...
    def test_build_pandajob_request(self):
        ...
        assert jobrequest.allowOtherCountry == request_dict[
            'allow_other_country']
