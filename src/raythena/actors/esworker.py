...
class ESWorker:
    ...
    def modify_job(self, job: PandaJob) -> PandaJob:
        ...
        if match:
            ...
            cmd = re.sub(
                r"--outputHITSFile=[0-9A-Z._]+\[[0-9,]+\].pool.root",
                f"--outputHITSFile={dummy_name}",
                cmd,
            )
    ...
    def stagein(self) -> None:
        ...
        try:
            ...
            if worker_logfile:
                log_to_file(
                    self.config.logging.get("level", "warning").upper(),
                    os.path.join(
                        self.payload_actor_process_dir,
                        os.path.basename(worker_logfile),
                    ),
                )
                disable_stdout_logging()
