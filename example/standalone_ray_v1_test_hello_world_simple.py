...
def build_nodes_resource_list(redis_ip):
    nodes = ray.nodes()
    resource_list = list()
    for node in nodes:
...
def main(args):
    print(f"args : {args}")
    redis_address = f"{args.redis_ip}:{args.redis_port}"
    ray.init(ignore_reinit_error=True,
             address="%s" % redis_address, _redis_password="%s" % args.redis_password)

    # show the ray cluster
    print(f"Ray Cluster resources : {ray.cluster_resources()}")
  
    # Get list of nodes to use
    nodes = build_nodes_resource_list(args.redis_ip)
